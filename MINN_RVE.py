...
class NN_strainRate_1ele(nn.Module):
    def __init__(self):
        super(NN_strainRate_1ele, self).__init__()
    ...
    def forward(self, x):
        ...
        other_vars = x[:, 24:25]  # delta t

        outputs = []
        for group in grouped_vars:
            # Combine each group of data with data from 24:25 to form 2 elements
...
def MINN_train(data, args, RESUME=0):
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    torch.cuda.empty_cache()
...
def MINN_train_gradient(data, args, RESUME=0):
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    torch.cuda.empty_cache()
    ...
    for epoch in range(initepoch, EPOCHS):
        for i, (data, labels) in enumerate(loader):
            ...
            # Enable gradient computation for data
            data.requires_grad_(True)
            
            outputs = ann(data) 
            
            # 1. Compute gradients of outputs with respect to data
            # Set grad_outputs to all ones, indicating derivative calculation with respect to outputs themselves
            grad = torch.autograd.grad(
                outputs=outputs,
                inputs=data,
                grad_outputs=torch.ones_like(outputs),
                create_graph=True,  # Retain computation graph for subsequent loss backpropagation
                retain_graph=True,
                only_inputs=True
            )[0]  # Gradient shape: [batch_size, data_dim]
            
            # 2. Extract the first 12 dimensions of the gradient
            grad_tau = grad[:, :12]  # Shape: [batch_size, 12]
            
            # 3. Construct loss condition for "gradient > 0"
            # When gradient <= 0, loss is |gradient| (penalize non-positive gradients); when gradient > 0, loss is 0
            loss_gradient = torch.mean(torch.clamp(-grad_tau, min=0.0))                 
            ...
            loss_stress = criterion(tau_labels,tau_upd)
            # loss_stress = criterion(labels,stress_upd)
            loss = 0.2*loss_stress + 0.8*loss_gradient
            optimizer.zero_grad()
            loss.backward()  # back pop
            optimizer.step()
...
if __name__ == "__main__":
    ...
    ## load data
    with h5py.File('dataset/train.h5', 'r') as hf:
        x_train = np.array(hf.get('x_train'))
        y_train = np.array(hf.get('y_train'))
                

    with h5py.File('dataset/validation.h5', 'r') as hf:
        x_test = np.array(hf.get('x_validation'))
        y_test = np.array(hf.get('y_validation'))

    
    loss_record,valLoss_record = train(x_train,y_train,
                            x_test,y_test, NORM=1,
                            SAVE=1, RESUME=0)
